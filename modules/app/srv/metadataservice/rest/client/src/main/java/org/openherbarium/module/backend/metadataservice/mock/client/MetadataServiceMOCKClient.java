...
public class MetadataServiceMOCKClient implements MetadataServiceMOCK, HasLogger {
  private static final String[] FIRST_NAMES =
      new String[] {"Sven", "Daniel", "Hendrik", "Horst", "Marco", "Markus", "Stefan", "Lukas"};
  private static final String[] LAST_NAMES =
      new String[] {"Müller", "Meier", "Knutsen", "Patton", "Leber", "Schuhmacher"};
  private static final String[] TAXON_NAMES_FIRST_PARTS =
      new String[] {"Carex", "Bartos", "Cranel"};
  private static final String[] TAXON_NAMES_SECOND_PARTS =
      new String[] {"Finea", "Rudea", "Flavella", "Bohemica"};
  private static final char[] LETTERS = new char[] {'A', 'B', 'C', 'D', 'E'};

  private static final List<Metadata> MOCK_DATA =
  ...
  private String targetURL() {
    final String targetURL =
        configuration.getMetaServiceUrl() + PATH_BASE + "/" + MetadataServiceMOCK.PATH_METHODE_FIND;
  ...
  private Stream<Metadata> getMetadataStreamFiltered(String determiner, String recorder,
      String taxon) {
    return MOCK_DATA.stream().filter(metadata -> filter(metadata.getDeterminer(), determiner))
        .filter(metadata -> filter(metadata.getRecorder(), recorder))
        .filter(metadata -> filter(metadata.getTaxonName(), taxon));
  }

  public boolean filter(String taxon, String taxonFilter) {
    if (taxonFilter == null || taxonFilter.equals("")) {
      return true;
    }
    return (StringUtils.isNotBlank(taxonFilter)
        && (StringUtils.containsIgnoreCase(taxon, taxonFilter)));
  }
  ...
  public boolean filter(Person person, String nameFilter) {
    ...
    return (StringUtils.isNotBlank(nameFilter)
        && (StringUtils.containsIgnoreCase(person.getFirstName(), nameFilter))
        || StringUtils.containsIgnoreCase(person.getLastName(), nameFilter));
  }
  ...
  private static List<Metadata> createMockData(int limit) {
    ...
    for (int i = 0; i < limit; i++) {
      ...
      metadata.setTaxonName(
          TAXON_NAMES_FIRST_PARTS[random.nextInt(TAXON_NAMES_FIRST_PARTS.length - 1) + 1] + " "
              + TAXON_NAMES_SECOND_PARTS[random.nextInt(TAXON_NAMES_SECOND_PARTS.length - 1) + 1]);
